...
# Required because the webapp starts first to provide authentication,
# but the seeder will fail if the roundcube tables are not available.
cat /home/roundcube.mysql.initial.sql | MYSQL_PWD=$DB_RC_PASSWORD mysql --protocol=socket -uroot -hlocalhost --socket="/run/mysqld/mysqld.sock" ${DB_RC_USERNAME}
